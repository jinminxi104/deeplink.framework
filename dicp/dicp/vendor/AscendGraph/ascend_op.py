...
class PromptFlashAttention(Operator):
    ...
    def infer_result(self, q, k, v, num_head, kv_head, seqlen, mask, head_dim):
        return torch.empty_like(q)

class AllGather(Operator):
    def __init__(self):
        super().__init__("AllGather")
...
class AllGatherMatmul(Operator):
    def __init__(self):
        super().__init__("AllGatherMatmul")

    def infer_result(self, x1, x2, group, gather_index, group_size):
        ...
        out_shape[0] = out_shape[0] * group_size

        # TODO handle gather at all dims in x

        return torch.empty(out_shape, dtype=out_dtype, memory_format=get_memory_format(x1))
